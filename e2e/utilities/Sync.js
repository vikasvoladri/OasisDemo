/**
 * Created by vikasv on 10/24/2015.
 */
...
var Sync = function () {
    ...
    this.isElementVisible = function (ele, timeout) {
        browser.waitForAngular();
        browser.wait(EC.visibilityOf(ele), timeout).catch(function (err) {
            throw new Error("Error occured while waiting for element with locator:" + ele.locator() + " to visible, Exception message:" + err + " stackTrace: " + err.stack);
        });
        return true;
    }
    ...
    this.isElementClickable = function (ele, timeout) {
        browser.waitForAngular();
        browser.wait(EC.elementToBeClickable(ele), timeout).catch(function (err) {
            throw new Error("Error occured while waiting for element with locator:" + ele.locator() + " to clickable, Exception message:" + err + " stackTrace: " + err.stack);
        });
        return true;
    }
    ...
    this.waitForPageToLoad = function (timeout) {
        browser.driver.wait(function () {
            return browser.driver.executeScript("return document.readyState").then(function (txt) {
                if (txt == "complete") {
                    return true;
                }
                else {
            ...
            throw new Error("Error while waiting for page to load, Exception message:" + err + " stackTrace: " + err.stack);
        });
    }
    this.switchToMetaMaskMainWindow = function () {
        browser.driver.getAllWindowHandles().then(function (windows) {
            browser.driver.switchTo().window(windows[0]);
        })
    };

    this.switchOasisMainWindow = function () {
        browser.driver.getAllWindowHandles().then(function (windows) {
    ...
    /*this.compareArrays = function(array1, array2) {
        if (array1.toString().toUpperCase() === array2.toString().toUpperCase())
            return true
        else
            return false
    }*/
